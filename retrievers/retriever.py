...
class Retriever():
    ...
    def maybe_retrieving_building_graph(self, source, word2idx, vocab_size, is_train=False):
        ...
        if self.opt.n_topic_words > 0:
            ...
            ref_tfidfs = [self.ranker.words_tfidf(d, self.opt.n_topic_words, word2idx, vocab_size) for d in
                          ref_doc_texts]

            # ref_doc_scores = list(range(len(ref_doc_texts_tokenized)))
            graph_utils = get_node_edge([source.split()] + ref_doc_texts_tokenized, [cur_tfidfs] + ref_tfidfs,
                                        ref_doc_scores)
        else:
            graph_utils = None
    ...
    def batch_maybe_retrieving_building_graph(self, source, word2idx, vocab_size, is_train=False):
        ...
        if self.opt.n_ref_docs > 0:
            if self.opt.random_search:
                ...
                batch_ref_doc_texts = [[self.ref_docs[ref_docid] for ref_docid in ref_docids] for ref_docids in
                                       batch_ref_docids]
            batch_ref_doc_texts_tokenized = [[d.split() for d in ref_doc_texts] for ref_doc_texts in
                                             batch_ref_doc_texts]
        else:
        ...
        if self.opt.n_topic_words > 0:
            ...
            # ref_doc_scores = list(range(len(ref_doc_texts_tokenized)))
            graph_utils = [get_node_edge([cur_doc_text.split()] + ref_doc_texts_tokenized, [cur_tfidfs] + ref_tfidfs,
                                         ref_doc_scores)
                           for cur_doc_text, ref_doc_texts_tokenized, cur_tfidfs, ref_tfidfs, ref_doc_scores in
                           zip(source,
