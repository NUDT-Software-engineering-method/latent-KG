...
class TopicSequenceGenrator(SequenceGenerator):
    def __init__(self,
                 model,
                 ntm_model,
                 use_topic_represent,
    ...
    def beam_search(self, src, src_lens, src_oov, src_mask, src_bow, oov_lists, word2idx, max_eos_per_output_seq=1):
        ...
        if self.use_topic_represent:
            src_bow_norm = F.normalize(src_bow)
            out = self.model.topic_model(src_bow_norm, encoder_final_state)

            prior_mean, prior_variance, posterior_mean, posterior_variance, posterior_log_variance, recon_x, topic_represent = out
            topic_represent = topic_represent.repeat(self.beam_size, 1)  # [batch * beam_size, topic_num]
        else:
            topic_represent = None
