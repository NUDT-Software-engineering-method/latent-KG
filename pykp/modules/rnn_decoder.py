...
class RNNDecoder(nn.Module):
    ...
    def from_opt(cls, opt):
        return cls(
            vocab_size=opt.vocab_size, embed_size=opt.word_vec_size, hidden_size=opt.decoder_size,
    ...
    def forward(self, y, topic_represent, h, memory_bank, src_mask, max_num_oovs, src_oov, coverage, **kwargs):
        """
...
class RNNDecoderTW(RNNDecoder):
    ...
    def from_opt(cls, opt):
        return cls(
            vocab_size=opt.vocab_size, embed_size=opt.word_vec_size, hidden_size=opt.decoder_size,
            num_layers=opt.dec_layers, memory_bank_size=opt.bidirectional * opt.enc_layers,
            coverage_attn=opt.coverage_attn, copy_attn=opt.copy_attention, bow_size=opt.bow_vocab,
            review_attn=opt.review_attn, pad_idx=opt.word2idx[pykp.io.PAD_WORD], attn_mode=opt.attn_mode,
            dropout=opt.dropout, use_topic_represent=opt.use_topic_represent, topic_attn=opt.topic_attn,
            topic_attn_in=opt.topic_attn_in, topic_copy=opt.topic_copy, topic_dec=opt.topic_dec, topic_num=opt.topic_num
        )
...
class RefRNNDecoder(RNNDecoder):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
                 review_attn, pad_idx, attn_mode, dropout=0.0, use_topic_represent=False, topic_attn=False,
                 topic_attn_in=False, topic_copy=False, topic_dec=False, topic_num=50):
        super(RefRNNDecoder, self).__init__(vocab_size, embed_size, hidden_size, num_layers, memory_bank_size,
                                            coverage_attn, copy_attn,
                                            review_attn, pad_idx, attn_mode, dropout=dropout,
                                            use_topic_represent=use_topic_represent,
                                            topic_attn=topic_attn,
