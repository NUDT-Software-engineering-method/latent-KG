...
import numpy as np
from pykp.context_topic_model.inference_network import CombinedInferenceNetwork, ContextualInferenceNetwork
...
class DecoderNetwork(nn.Module):
    ...
    def forward(self, x, latent_representation):
        ...
        h_ = F.softmax(self.reparameterize(posterior_mu, posterior_log_sigma), dim=1)
        h = self.drop_h(h_)
        ...
        return h, h_, word_dist, (posterior_mu, posterior_sigma, posterior_log_sigma), (
    ...
    def print_topic_words(self, vocab_dic, fn, n_top_words=10):
        beta_exp = self.topic_word.detach().cpu().numpy()
